...
mod tests {
    ...
    fn test_load() {
        ...
        vm.program = vec![
            // LOAD 0, #16909320
            u8::from(Opcode::LOAD), 0x00, 0x01, 0x02, 0x04, 0x08,
            u8::from(Opcode::HALT)
        ];
        vm.run();
    ...
    fn test_load_zero() {
        ...
        vm.program = vec![
            // LOAD 0, #0
            u8::from(Opcode::LOAD), 0x00, 0x00, 0x00, 0x00, 0x00,
            u8::from(Opcode::HALT)
        ];
        vm.run();
        assert_eq!(vm.registers[0], 0);
    ...
    fn test_jmp() {
        ...
        vm.program = vec![
            // JMP #12 (i.e. LOAD 0, #2)
            u8::from(Opcode::JMP), 0x00, 0x00, 0x00, 0x0C,
            // LOAD 0, #1
            u8::from(Opcode::LOAD), 0x00, 0x00, 0x00, 0x00, 0x01,
            u8::from(Opcode::HALT),
            // LOAD 0, #2
            u8::from(Opcode::LOAD), 0x00, 0x00, 0x00, 0x00, 0x02,
            u8::from(Opcode::HALT)
        ];
        vm.run();
        assert_eq!(vm.registers[0], 2);
    ...
    fn test_je_zero() {
        ...
        vm.program = vec![
            // JE #12 (i.e. LOAD 0, #2)
            u8::from(Opcode::JE), 0x00, 0x00, 0x00, 0x0C,
            // LOAD 0, #1
            u8::from(Opcode::LOAD), 0x00, 0x00, 0x00, 0x00, 0x01,
            u8::from(Opcode::HALT),
            // LOAD 0, #2
